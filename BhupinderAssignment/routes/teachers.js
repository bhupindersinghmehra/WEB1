const db=require('../makedatabase').Teacher
...
rout.route("/:id").get((req,res)=>{
    ...
    db.findOne({where:{ teacherid :req.params.id}}).
    then(result=>{
        if(result){
            res.json(result);
        }
...
.put((req,res)=>{
    ...
    db.update(req.body,{where :{teacherid: req.params.id}})
    .then(result=>{db.findOne({where:{teacherid:req.params.id}}).then(ress=>{res.send(ress);console.log(ress)})})
...
db
.destroy({where:{teacherid:req.params.id}})
